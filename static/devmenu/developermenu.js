var DeveloperMenu = {
	...
	Menu: {
				...
				success: function (req) {
						...
						if (req.responseText) {
							...
							if (sdApps.apps && sdApps.apps.length > 0) {
								...
								this.updateKbnImage();
								this.loadFile({
									url: DeveloperMenu.Util.getAppBaseUrl() + '/gxmetadata/settings.json?' + Math.random(),
									success: function (reqSettings) {
										var settings = {}
										if (reqSettings.responseText) {
											if (window.JSON) {
												settings = JSON.parse(reqSettings.responseText);
											}
											else {
												eval("settings = " + reqSettings.responseText);
											}
											var gpn = settings.Config.GeneratorProperties['@ANDROID_EXEC_TYPE'] === "GeneXus Project Navigator"
											if (gpn)
												this.renderAndroidGpnMenu();
											else
									...
									failure: function() { this.renderAndroidMenu(sdApps); },
									scope: this
								})
							}
							else {
		...
		renderAndroidGpnMenu: function () {
			var androidMenuElement = DeveloperMenu.Dom.el("sdapps-box-content"),
				appUrl = DeveloperMenu.Util.getAppBaseUrl(),
				qrCodeUrl = DeveloperMenu.Util.getQRCodeImage(appUrl);				

			var html = [
				'<div class="action-box qrcode-box">',
					'<figure>',
						'<img class="qrcode" src="', qrCodeUrl, '" alt="QRCode for Android"/>',
						'<figcaption>',
							'<header>',
								'<h2>QRCode for Android</h2>',
							'</header>',
							'<a href="', appUrl, '" target="_blank">Services URL for GPN</a>',
						'</figcaption>',
					'</figure>',
				...
				'<div class="action-box download-box">',
					'<figure>',
						'<a href="https://play.google.com/store/apps/details?id=com.genexus.android.navigator" target="_blank">',
							'<img id="kbn-icon" src="devmenu/KBN.png" alt="Download GeneXus Project Navigator"/>',
						'</a>',
						'<figcaption>',
							'<header>',
								'<h2>GeneXus Project Navigator</h2>',
							'</header>',
							'<p>The GPN app makes native Android online apps prototyping simple. <a href="https://play.google.com/store/apps/details?id=com.genexus.android.navigator" target="_blank">Download</a></p>',
						'</figcaption>',
					'</figure>',
				'</div>'
			].join("");

			androidMenuElement.innerHTML = html;
		},
